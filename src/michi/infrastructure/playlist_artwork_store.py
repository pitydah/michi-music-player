...
def _is_decodable_image(path: Path) -> bool:
    """True only when the file REALLY decodes as an image (P1-01)."""
    try:
...
class FilesystemPlaylistArtworkStore(PlaylistArtworkStorePort):
    ...
    def _prepare_variant(
        ...
        """IMMUTABLE CANDIDATE PROTOCOL (P0-03):
        1. prepare writes a content-versioned FINAL managed file that
           EXISTS before any database reference;
        2. DB failure deletes the new candidate best-effort and keeps old;
        3. DB success makes the new reference authoritative;
        4. old managed asset cleanup is best-effort after commit.
