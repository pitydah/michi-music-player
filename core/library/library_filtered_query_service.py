...
class LibraryFilteredQueryService:
    ...
    def fetch_artists(self, offset: int = 0, limit: int = 100,
        ...
        sql = (
            ...
            f"= {_NORM_ARTIST} GROUP BY cover.album_key "
            "ORDER BY EXISTS(SELECT 1 FROM album_art_cache cache "
            "WHERE cache.album_hash = cover.album_key) DESC, "
            "COUNT(*) DESC, MAX(COALESCE(cover.year, 0)) DESC, "
            "cover.album_key COLLATE NOCASE ASC LIMIT 1) AS cover_key "
            "FROM media_items m WHERE m.deleted_at IS NULL "
