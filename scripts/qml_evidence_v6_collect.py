#!/usr/bin/env python3
"""QML Evidence V6 Collector — collect markers, JUnit data, and produce evidence with dimensions."""
import argparse
import ast
import json
import subprocess
...
def main():
    ...
    outpath = ARTIFACTS / "qml-evidence-v6.json"
    outpath.write_text(json.dumps(evidence, indent=2, default=str))
    print(f"Evidence V6 written to {outpath}")
    print(f"SHA: {sha}")
