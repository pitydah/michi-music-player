#!/usr/bin/env python3
"""Generate migration manifest V3 from pytest collection + surface inventory."""
import json
...
def main():
    ...
    out = REPO / "docs" / "qml_migration_manifest_v3.json"
    out.write_text(json.dumps(manifest, indent=2, ensure_ascii=False) + "\n")
    print(f"Generated V3 manifest: {len(manifest['modules'])} modules, {sum(len(m['evidence']['unit_tests']) for m in manifest['modules'])} tests matched")
    print(f"Wrote {out}")
