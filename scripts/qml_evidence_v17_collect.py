...
def compute_scores(junit: dict, issues: list[str]) -> dict:
    ...
    return {"test_score": test_score, "placeholder_score": placeholder_score,
...
def main():
    ...
    scores = compute_scores(junit, issues)
    evidence = {
        "version": "v17",
        "sha": actual, "expected_sha": expected,
    ...
    out = repo / "artifacts" / "qml-evidence-v17.json"
    out.parent.mkdir(parents=True, exist_ok=True)
    with open(out, "w") as f:
        json.dump(evidence, f, indent=2)
    print(f"Evidence V17: {out}")
