...
| Persistence    | M11.2A read-only health inspection + M11.2B validated LKG backup / staged recovery         | Tested at capability level (M11.2A + M11.2B); startup wiring pending M11.2D |
| Tests          | 306 pytest tests passing; CI green (lint/test/build, offscreen)                            | Verified                                                                    |
