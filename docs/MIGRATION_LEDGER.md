...
3. Classification is exact, once per atomic responsibility: ADAPT, REWRITE, KEEP (governance responsibility only), RETIRED (product decision — capability withdrawn; successor capability may be ACTIVE), RETAINED (future product capability — not part of the current refactor), or DISCARD. SPLIT is used only with separately named child responsibilities. A row whose prior material contains no such concept at all is marked N/A — no concept to classify.
...
| ML-110 | Distributed/ecosystem scope: AI assistant, sync, radio, recognition, home audio | Legacy tree contains `core/ai/`, `core/lyrics/`, `core/home_audio_service.py`, `core/context/providers/snapshot/radio.py`, `recognition.py`, Android sync (`android/`) | RETAINED (future product capability — AFTER PLAYER STABLE) | Local-first, audio-only 1.0 current refactor; distributed/ecosystem scope is RETAINED and scheduled AFTER PLAYER STABLE (MASTER_ROADMAP_1.0.md Product Scope + POST_1_0_BACKLOG.md). No migration action in the current refactor. |
