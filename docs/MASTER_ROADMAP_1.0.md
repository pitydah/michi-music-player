...
| M5 Database/Settings         | TESTED     | Settings persistence (SQLite, WAL) through `SettingsRepository`/`SQLiteSettingsRepository`; restart gate                                              | ORIGINAL CONTRACT CLOSED — durable session persistence (queue/position/current-track/repeat/shuffle-seed/geometry/theme + schema versioning + migration framework) TESTED (C1-C8, 765-pass suite). |
