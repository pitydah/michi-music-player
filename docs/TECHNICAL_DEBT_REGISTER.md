...
TD-008 (current era) is also resolved — see its resolution note below.
...
| TD-008 | Queue↔Playback atomicity: `QueueService.play_index`/`next`/`previous` mutated `current_index` before `PlaybackService.load_and_play` completed. A playback failure (missing file, unsupported format) could leave queue and playback divergent — the queue pointing at a track playback never loaded. | RESOLVED — confirm-after-effect: `QueueService.play_index` commits `current_index` only after `load_and_play` succeeds; `next`/`previous` route through the same atomic path; exceptions propagate. Regression-tested by failure injection (`tests/test_queue_service.py::TestNavigationFailureAtomicity`, `tests/test_convergence.py::TestEndOfMedia::test_auto_advance_failure_preserves_index`); full suite 162 passed. |
...
3. SIGNIFICANT debt blocks 1.0: TD-008 was the only open SIGNIFICANT entry and is now RESOLVED (see Resolved Debt). Required-1.0 capability gaps (TD-010/TD-011) are not debt; they are tracked by the canonical 1.0 contract in MASTER_ROADMAP_1.0.md and must close before release per their scheduled work packages.
