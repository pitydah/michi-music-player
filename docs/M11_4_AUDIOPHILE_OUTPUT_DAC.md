...
- `DAC-V35-100R1.3.4` final native edge seal — **CLOSED-AUTOMATED /
  ...
  authority check, unproven compensation is retained as an observable retryable
  receipt, and a stale successful detach reconciles the exact resource without
  touching a newer generation's watch. Native truth
  (``watch_installed``/``remove_watch_count``/``attached``/``destroyed``) is
  asserted by the gates.
- `DAC-V35-100R1.3.2` native closure final corrective — **CLOSED-AUTOMATED /
  GO; PUBLISHED**. The RUNNING context-command commit
...
- `DAC-V35-100R1.3` playback compatibility, output-mode separation, and native
  ...
  bounded ordered candidate set, and Compatible Direct may use a wider lossless
  container only when rate, channels and significant bits are preserved
  (sample-level proof: `s32 == s16 << 16`; the single `audioconvert` runs with
  dithering and noise shaping explicitly disabled). Output refusals now reach
  the Audio Output surface as one typed truth with explicit recovery
  (`Try Compatible Direct` / `Use Shared` / `Cancel`, never automatic).
  Timed-out context commands are abandoned and cannot mutate state, `close()`
  releases the Direct executor before claiming closure, and the container keeps
  an explicit lifecycle state. Field smoke on the real SMSL resolved
  `S16_LE rejected -> S32_LE opened` and reached `running direct`.
