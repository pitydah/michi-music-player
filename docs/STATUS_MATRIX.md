...
| M5 Database/Settings         | PARTIAL    | M5 ORIGINAL CLOSEOUT in progress — recovering durable session persistence (queue/position/current-track/repeat/shuffle-seed/geometry/theme + schema versioning + migration framework); TESTED only at the final M5 gate.|
