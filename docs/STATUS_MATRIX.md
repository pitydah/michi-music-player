...
| M4 Queue                     | TESTED     | Queue basics, TD-015 pending identity, TD-016 cancellation terminal, repeat modes (none/one/all) and deterministic shuffle navigation — all TESTED (a977378, f93a6a8; 441-pass suite). Reorder is Post-1.0 (not a blocker). |
...
| M6 Library                   | TESTED     | Scan + filesystem degradation diagnostics (TD-013) + search + canonical basic metadata extraction (title/artist/album/duration via Mutagen behind MetadataExtractorPort, per-file fallbacks, failure-safe) — TESTED (5fd8ec3; 460-pass suite). Local product experience (canonical music model, artwork, albums/artists/genres/folders, PathView, favorites, history, playlists, local mix, final player UI) is scheduled as LOCAL-01..08 pre-Beta. |
...
Transitions pending per the canonical 1.0 contract: all components with outstanding Required-1.0 gaps must reach TESTED before M15. Currently that is none: M4 and M6 are TESTED; the local product experience (LOCAL-01..08) is scheduled pre-Beta. M11.2A-E persistence recovery is COMPLETE for Required 1.0; TD-016 (Queue/Playback cancellation-terminal synchronization) is RESOLVED; LOCAL-01 (Canonical Music Model) is DONE (ba1532d); LOCAL-02 (Artwork Pipeline) is DONE (9b16504); LOCAL-03 (Rich Library Views) is DONE (e7416ac); LOCAL-04 (PathView) is DONE (635b35b); LOCAL-05 (Favorites / History) is DONE (938191b); LOCAL-06 (Playlists) is DONE (ccdbc85); LOCAL-STABILIZATION-01 is DONE (5 commits, 584-pass suite); the next authorized work package is LOCAL-META-02 (Rich Canonical Metadata) per MASTER_ROADMAP_1.0.md.
