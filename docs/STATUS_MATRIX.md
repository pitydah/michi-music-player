...
| M6 Library                   | PARTIAL    | M6 ORIGINAL CONTRACT IN PROGRESS (M6.0-M6.8 per docs/M6_LIBRARY_MASTER_PLAN.md): the reduced-contract TESTED status is superseded; canonical model v2, persistent index, incremental engine, async pipeline, artwork v2, canonical projections and QML decomposition are the open phases. (Scan + TD-013 diagnostics + search + canonical basic metadata remain TESTED — 5fd8ec3; 460-pass suite.) M6.1 Canonical Music Model v2 DONE (8694077): canonical identities, canonical ordering, composer model, album v2 fields, timeline derivation in domain. M6.2 Persistent Library Index DONE (c26acfd): versioned bounded-context index repository + strict metadata codec; filesystem remains the truth of physical existence. M6.3 Incremental Library Engine DONE (dd701e7): fingerprint delta classification, no reparse of unchanged, atomic commit. M6.4 Async Library Pipeline DONE (0dc2566): scan off the UI thread, generation supersession (stale never commits), cooperative cancellation, progress contract; TD-009 owned by M6.4. M6.5 Artwork Pipeline v2 DONE (0736534): front-cover preference, local fallback, digest-aware cache invalidation. M6.6 Canonical Library Projections DONE (44b1471): one canonical album model for the six views, key-based selection with safe clear on deletion, canonical numbers in album tracks. M6.7 Library Presentation Architecture DONE (b9b243b): monolith decomposed into modular Loader-driven components; TD-017 resolved. |
...
Transitions pending per the canonical 1.0 contract: all components with outstanding Required-1.0 gaps must reach TESTED before M15. Currently M6 is PARTIAL — its original M6 contract (M6.0-M6.8 per docs/M6_LIBRARY_MASTER_PLAN.md) supersedes the reduced-contract TESTED status; M4 is TESTED. M11.2A-E persistence recovery is COMPLETE for Required 1.0; TD-016 (Queue/Playback cancellation-terminal synchronization) is RESOLVED; LOCAL-01 (Canonical Music Model) is DONE (ba1532d); LOCAL-02 (Artwork Pipeline) is DONE (9b16504); LOCAL-03 (Rich Library Views) is DONE (e7416ac); LOCAL-04 (PathView) is DONE (635b35b); LOCAL-05 (Favorites / History) is DONE (938191b); LOCAL-06 (Playlists) is DONE (ccdbc85); LOCAL-STABILIZATION-01 is DONE (5 commits, 584-pass suite); LOCAL-META-02 (Rich Canonical Metadata) is DONE (4 commits, 619-pass suite); the next authorized work package is M6.0 — ORIGINAL CONTRACT RECONCILIATION (in progress), followed by M6.1-M6.8 per docs/M6_LIBRARY_MASTER_PLAN.md.
