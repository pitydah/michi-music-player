...
| M11.3 Multi-Engine Audio Runtime | AUDITED | Not implemented. Evidence collected (RESEARCH-01 — AUDIO_ENGINE_REFERENCE_AUDIT.md: engine abstraction, gapless, setformat, MPD output policy, allowed_formats); state/risks documented in docs/M11_3_MULTI_ENGINE_AUDIO_RUNTIME.md (AudioPort stays transport-only; engine registry/lifecycle and DAC/output config in separate ports; MPD private-process contract; binding decisions pending in M11.3A). Work package M11.3 — READY. |
...
| -------------------------------- | ---------- | ----- |
| M8-R1 Playlists First-Class Navigation | READY | Next authorized WP. Stable opaque `playlist_id` + backward-compatible migration of existing persisted playlists; navigation route/semantics; Queue/Playlist separation preserved. Component counterpart: M8 Navigation (TESTED) reopened scoped. |
| M9-R1 Playlists Sidebar / Presentation | READY | Controlled M9 reopening (pre-authorized); Sidebar playlists section, All Playlists, bounded pinned/recent, create affordance, single canonical playlist screen; Design Canon 2.0 preserved. Component counterpart: M9 Premium Presentation System (FROZEN) reopened scoped. |
| M11.3 Multi-Engine Audio Runtime | READY | AudioPort stays transport-only; engine registry/lifecycle and DAC/output config in separate ports/services; Qt reference + GStreamer + MPD managed adapters; quiescent switching. |
