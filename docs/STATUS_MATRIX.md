...
| M11.3 Multi-Engine Audio Runtime | AUDITED | Overall INCOMPLETE (C–G outstanding) but M11.3A + M11.3A-R1 + M11.3B + M11.3B-R1 IMPLEMENTED/TESTED/FROZEN (M11.3B: AudioTransportRouter PRODUCTIVELY WIRED — Qt reference engine behind the router, single owned backend, engine state READY, test graph == production graph; M11.3B-R1: single canonical Qt provider (registry identity), transactional startup PROBE→INITIALIZING→OPEN→BIND→VALIDATE→READY with UNAVAILABLE/FAILED convergence + cleanup, ServiceGraph bound_audio_port non-authority): domain engine contracts, AudioTransportRouter (productive with Qt as reference provider since M11.3B), AudioEngineRegistry, AudioEngineProviderPort, AudioEngineService (sole AudioEngineState), activation/lifecycle semantics, Qt provider ownership, GStreamer/MPD availability probes (installed != implemented), ADR 0007. Outstanding: C (GStreamer adapter), D (MPD adapter), E (availability runtime), F (selection/persistence), G (failure convergence). Work package M11.3 — IN PROGRESS. Work package M11.3 — IN PROGRESS. |
