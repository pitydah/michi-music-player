...
| M9 Premium Presentation System | PARTIAL  | Approved 2026-08-20. M9.0–M9.4 architecture/design-system/control/gallery/shell implementation is under review; Library/search/error-state migration started. Remaining M9.5–M9.15 acceptance is tracked in `docs/M9_PREMIUM_PRESENTATION_SYSTEM.md`. |
...
Transitions pending per the canonical 1.0 contract: all components with outstanding Required-1.0 gaps must reach TESTED before M15. M6 is TESTED / ORIGINAL CONTRACT CLOSED (M6.0-M6.8, beff4a0/01fe2ec); M7 SEARCH is TESTED / CLOSED / FROZEN (canonical local search, M6 shared album-artist semantics, title-first album ranking, five entity result sets, six-view same-AlbumIds invariant, 10k correctness baseline, FTS Post-1.0 — per docs/M7_SEARCH_MASTER_PLAN.md); M11.2A-E persistence recovery is COMPLETE for Required 1.0; TD-016 (Queue/Playback cancellation-terminal synchronization) is RESOLVED; LOCAL-01 through LOCAL-06 and LOCAL-META-02 are DONE. The approved next WP is M9 Premium Presentation System per `MASTER_ROADMAP_1.0.md`; M12 follows its closeout.
