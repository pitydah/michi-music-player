...
CAPABILITY_STATE_KEYS = {
    ...
    "connections", "connections_michilink", "home_audio", "snapcast",
    "devices", "devices_sync", "radio", "playlists", "eq",
    "output_profiles", "settings", "audio_lab", "metadata",
    "smart_tagging", "disc_lab", "library_doctor", "diagnostics",
    "ai", "michi_ai", "theme", "navigation", "route_registry",
    "app_state", "command_palette", "cover", "notifications",
    "global_search", "transmit",
}
...
class CapabilityBridge(QObject):
    ...
    def refresh(self):
        ...
        for key in CAPABILITY_STATE_KEYS:
            ...
            if key not in caps:
                caps[key] = "available" if bridge_key in bridge_keys else "unavailable"
