...
class CommandPaletteBridge(QObject):
    ...
    def getActionDetail(self, command_id: str):
        ...
        return {
            ...
            "requires_context": action.requires_context,
            "argument_schema": list(action.argument_schema),
    ...
    def executeCommand(self, command_id: str):
        ...
        if action.argument_schema:
            return {"ok": False, "code": "PARAMETERS_REQUIRED",
    ...
    def executeCommandWithParams(self, command_id: str, params_json: str):
        """Execute a command with explicit parameters via an ActionContext."""
        import json
        ...
        if action.argument_schema:
            missing = [arg for arg in action.argument_schema if arg not in params]
            if missing:
                return {"ok": False, "code": "PARAMETERS_MISSING",
                        "missing": missing,
        ...
        context = ActionContext(
            ...
            public_ref=str(params.get("public_ref") or ""),
            parameters={k: v for k, v in params.items()
                        if k not in ("entity_type", "entity_id", "public_ref")},
            source_component="command_palette",
        )
        self._add_recent(command_id)
        return self._registry.execute(command_id, context)
