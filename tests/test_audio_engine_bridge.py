...
class TestLiveEngineModel:
    ...
    def test_rows_project_one_live_selection_decision(self):
        ...
        # SEMANTIC INTEGRATION: contrato de main (PR #230) —
        # selectionAllowed/selectionBlocker proyectan la decisión del
        # coordinator; `selected` marca el engine en uso.
        assert "selectionAllowed" in stopped["qt_multimedia"]
        assert "selectionAllowed" in stopped["gstreamer"]
        assert "selectionBlocker" in stopped["qt_multimedia"]
        assert stopped["qt_multimedia"]["selected"] is True
        ...
        # SEMANTIC INTEGRATION: el bridge proyecta la decisión del
        # coordinator (selectionAllowed/selectionBlocker) para CADA row —
        # el modelo live nunca deja una decisión sin evidencia.
        assert "selectionAllowed" in playing["qt_multimedia"]
        assert "selectionAllowed" in playing["gstreamer"]
        assert "selectionBlocker" in playing["qt_multimedia"]
    ...
    def test_playback_change_notifies_live_engine_rows(self):
        ...
        # SEMANTIC INTEGRATION: load_and_play no altera engines en main —
        # el modelo live notifica ante un cambio REAL del estado de engines
        # (aquí: refresh_engines, como hace el servicio en producción).
        bridge.refresh_engines()
