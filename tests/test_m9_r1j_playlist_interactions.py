...
class _QmlErrors:
    ...
    def __init__(self, extra_watched=()):
        from PySide6.QtCore import qInstallMessageHandler

        self.messages = []
        self._previous = None
        self._watched = self.WATCHED + tuple(extra_watched)

        def handler(msg_type, context, message):
            text = str(message)
            file_name = str(getattr(context, "file", "") or "")
            if any(w in file_name for w in self._watched) and any(
                token in text
                for token in (
                    "ReferenceError",
                    "TypeError",
...
def _load_path(engine, path, errors, label=""):
    """M9-R1M harness lifecycle — semantic ordering is load-bearing:

    A. finish deferred teardown from PREVIOUS tests (their async deleteLater
       emits "of null" while destroying foreign engines);
    B. clear ONLY messages that existed BEFORE this load starts;
    C. construct the CURRENT component (compile errors asserted);
    D. create the CURRENT object (runtime errors must be preserved);
    E. let Component.onCompleted and initial bindings execute;
    F. read errors generated by THIS component;
    G. they MUST NOT be silently discarded.

    INVARIANT: after component construction there is NEVER an unasserted
    errors.drain()."""
    # A. prior teardown finishes with the default handler semantics: the
    # async deleteLater of previous tests only executes when Python releases
    # their engines (GC), so collect + process before clearing anything.
    for _ in range(3):
        _process()
    import gc
    ...
    for _ in range(3):
        _process()
    # B. stale noise (pre-existing messages) may be discarded
    errors.drain()

    # C. construct the CURRENT component
    component = QQmlComponent(engine, str(path))
    errs = "; ".join(e.toString() for e in component.errors())
    assert component.status() == QQmlComponent.Ready, f"{label}: {errs}"

    # D. create the CURRENT object
    obj = component.create()
    assert obj is not None, f"{label}: null object"
    ...
    # E. let onCompleted and initial bindings execute
    _process()

    # F+G. errors generated by THIS component are inspected and asserted
    runtime = errors.drain()
    assert runtime == [], f"{label}: runtime errors: {runtime}"
    return obj


def _load(engine, rel, errors):
    return _load_path(engine, QML_DIR / rel, errors, label=rel)
...
class TestDetailActionsRuntime:
    ...
    def test_detail_rename_action_opens_shared_dialog(self, tmp_path, qapp):
        ...
        try:
            ...
            errors.restore()
            engine.deleteLater()
            # procesar el teardown con el handler DEFAULT para que sus
            # mensajes "of null" no contaminen el siguiente test
            for _ in range(5):
                _process()
...
class TestQmlHarnessIntegrity:
    """M9-R1M: the harness must distinguish stale pre-load messages from
    runtime errors emitted by the CURRENT component — a deliberate runtime
    ReferenceError probe must be detected, never silently discarded."""

    PROBE_SOURCE = """import QtQuick
Item {
    Component.onCompleted: {
        harnessProbeMissingObject.value = 1
    }
}
"""

    def _probe(self, tmp_path):
        probe = tmp_path / "QmlHarnessProbe.qml"
        probe.write_text(self.PROBE_SOURCE)
        return probe

    def test_load_preserves_current_component_creation_errors(self, tmp_path, qapp):
        """component Ready → create → runtime ReferenceError in
        Component.onCompleted → the harness MUST observe and reject it."""
        world = _world(tmp_path)
        engine = _engine(world)
        errors = _QmlErrors(extra_watched=("QmlHarnessProbe.qml",))
        probe = self._probe(tmp_path)
        try:
            with pytest.raises(AssertionError, match="runtime errors") as exc:
                _load_path(engine, probe, errors, label="probe")
            # the asserted failure text must name the missing identifier
            # (the drain consumes messages; the exception carries the text)
            assert "harnessProbeMissingObject" in str(exc.value), (
                f"probe error no menciona el identificador: {exc.value}"
            )
        finally:
            errors.restore()
            engine.deleteLater()
    ...
    def test_load_clears_preexisting_but_preserves_current_errors(self, tmp_path, qapp):
        """Boundary: stale messages (pre-existing noise) are discarded before
        the current load; errors DURING the current load are preserved."""
        world = _world(tmp_path)
        engine = _engine(world)
        errors = _QmlErrors(extra_watched=("QmlHarnessProbe.qml",))
        probe = self._probe(tmp_path)
        try:
            # stale noise prepopulated by a previous (simulated) teardown
            errors.messages.append(
                "file:///stale/Other.qml:1: TypeError: Cannot read property 'x' of null"
            )
            # valid load must NOT fail on the stale message
            obj = _load(engine, "patterns/SearchOverlay.qml", errors)
            assert obj is not None
            # broken probe load MUST fail on its CURRENT runtime error
