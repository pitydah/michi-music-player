...
class TestAudioSettingsMigrator:
    ...
    def test_migrate_legacy_keys_dry_run_does_not_write(self):
        with (
            patch("audio.settings.audio_settings_migrator.get") as mock_get,
            patch("audio.settings.audio_settings_migrator.set_") as mock_set,
        ):
            def fake_get(key):
                legacy = {
                    "playback/replaygain": True,
    ...
    def test_migrate_all_runs_both(self):
        with (
            patch("audio.settings.audio_settings_migrator.get") as mock_get,
            patch("audio.settings.audio_settings_migrator.set_"),
        ):
            mock_get.return_value = None
