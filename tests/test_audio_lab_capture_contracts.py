...
def test_bridge_reuses_same_adc_recorder_for_whole_session(tmp_path):
    ...
    assert [call[0] for call in recorder.calls] == [
        "detect", "detect", "start", "pause", "marker", "stop"
    ]


def test_bridge_area_navigation_uses_registered_backup_route():
    navigation = FakeNavigation()
    bridge = AudioLabBridge(audio_lab_service=FakeAudioLabService(), navigation_bridge=navigation)
...
def test_capture_capabilities_are_truthful():
    ...
    assert bridge.getCaptureCapabilities() == {
        "available": True,
        "formats": ["wav", "flac"],
        "dsp_filters": ["riaa_eq", "declicker"],
    }
...
def test_removed_simulated_pages_are_not_present():
    assert not Path("ui_qml/pages/audio_lab/ADCRecorderPage.qml").exists()
    assert not Path("ui_qml/pages/audio_lab/CDRipperPage.qml").exists()
