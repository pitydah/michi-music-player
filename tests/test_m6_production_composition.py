...
class TestProductionComposition:
    def test_production_graph_wires_library_index(self, tmp_path):
        ...
        assert (
            graph.playlist_service.playlists == ()
        )  # canonical service authority (bridge is composed in ApplicationContainer)
    ...
    def test_production_playlist_survives_restart(self, tmp_path):
        ...
        db_path, graph = _make_graph(tmp_path, StatScanner([a, b]), extractor)
        graph.library.scan(str(music))
        graph.playlist_service.create_playlist("Road")
        road_id = graph.playlist_service.playlists[0].playlist_id
        graph.playlist_service.add_track(road_id, str(a))
        graph.playlist_service.add_track(road_id, str(b))
        assert [
            (p.name, len(p.track_paths)) for p in graph.playlist_service.playlists
        ] == [("Road", 2)]

        _teardown_graph(graph)
        _, graph2 = _make_graph_at(db_path, StatScanner([a, b]), CountingExtractor())
        assert [
            (p.name, len(p.track_paths)) for p in graph2.playlist_service.playlists
        ] == [("Road", 2)]
    ...
    def test_production_bridge_exposes_persisted_playlists(self, tmp_path):
        ...
        graph.library.scan(str(music))
        graph.playlist_service.create_playlist("Keep")
        graph.playlist_service.add_track(
            graph.playlist_service.playlists[0].playlist_id, str(a)
        )
        assert graph.playlist_service.playlists != []  # real service, not a no-op
...
class TestProductionGolden:
    def test_production_golden_full_sequence(self, tmp_path, qapp):
        ...
        graph.playlist_service.create_playlist("Golden")
        golden_id = graph.playlist_service.playlists[0].playlist_id
        for p in paths[:3]:
            graph.playlist_service.add_track(golden_id, str(p))
