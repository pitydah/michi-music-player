...
def _make_bridge(db):
    """Build the bridge with the injected scan repository (ADR-003)."""

    repo = LibraryDoctorScanRepository(_FakeDB(db))
    return LibraryDoctorBridge(db=_FakeDB(db), scan_repository=repo)
