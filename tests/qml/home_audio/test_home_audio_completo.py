"""Test HomeAudioBridge completo — zones, groups, streams, volume, mute, latency,
reconnect, server handoff, playback transfer, offline, partial failure.
Contractual adapters: Snapcast + Home Assistant."""
...
class TestServerHandoff:
    def test_handoff_snapcast(self, bridge, mock_snapcast):
        bridge.refresh()
...
class TestPlaybackTransfer:
    def test_transfer_to_zone(self, bridge, mock_snapcast):
        result = bridge.playbackTransfer("group1")
        assert result["ok"] is True
        mock_snapcast.playback_transfer.assert_called_once_with("group1")

    def test_transfer_empty_target(self, bridge):
        result = bridge.playbackTransfer("")
        assert result["ok"] is False
        assert result["error"] == "EMPTY_TARGET"

    def test_transfer_no_snapcast(self):
        b = HomeAudioBridge(ha_controller=None, snapcast_ctrl=None)
        result = b.playbackTransfer("group1")
        assert result["ok"] is False

    def test_transfer_unknown_zone(self, bridge):
        result = bridge.playbackTransfer("nonexistent")
        assert result["ok"] is True or result["ok"] is False
