...
"""Tests for AlbumCoverFlowView model lifecycle and delegate rendering."""
...
QML_DIR = (
    pytest.importorskip("pathlib").Path(__file__).resolve().parent.parent.parent.parent
    / "ui_qml"
)


class FlatAlbumModel(QAbstractListModel):
    """Minimal QAbstractListModel for PathView testing without async paging."""
...
def _load_coverflow(engine, model=None):
    ...
    component.loadUrl(
        QUrl.fromLocalFile(
            str(QML_DIR / "pages/library/album/AlbumCoverFlowView.qml")
        )
    )
    ...
    obj._qml_component_ref = component
...
class TestAlbumCoverFlowView:
    def test_empty_model(self, engine):
        ...
        try:
            path_view = view.findChild(QObject, "albumCoverFlowPathView")
    ...
    def test_single_album(self, engine):
        ...
        try:
            ...
            assert model.data(index, model.TitleRole) == "Alpha"
            assert model.data(index, model.ArtistRole) == "X"
            assert model.data(index, model.YearRole) == 2000
        finally:
            view.deleteLater()
    ...
    def test_multiple_albums(self, engine):
        ...
        try:
            ...
            for position, expected in enumerate(albums):
                index = model.index(position, 0)
                assert model.data(index, model.TitleRole) == expected["title"]
                assert model.data(index, model.ArtistRole) == expected["artist"]
        finally:
            view.deleteLater()
    ...
    def test_delegate_shows_cover(self, engine):
        ...
        try:
            index = model.index(0, 0)
            assert model.data(index, model.CoverKeyRole) == "cover_abc"
            assert model.data(index, model.TitleRole) == "CoverTest"
        finally:
            view.deleteLater()
    ...
    def test_model_append(self, engine):
        ...
        try:
            ...
            assert model.rowCount() == 1
            index = model.index(0, 0)
            assert model.data(index, model.TitleRole) == "Appended"
        finally:
            view.deleteLater()
    ...
    def test_model_remove(self, engine):
        ...
        try:
            ...
            assert model.rowCount() == 1
            index = model.index(0, 0)
            assert model.data(index, model.TitleRole) == "Second"
        finally:
            view.deleteLater()
    ...
    def test_paged_model_interface(self, engine):
        ...
        query_service.count_albums.return_value = 2
        query_service.fetch_albums.return_value = [
        ...
        paged = AlbumPagedListModel(query_service=query_service)
        paged.refresh()
        ...
        try:
            ...
            index = paged.index(0, 0)
            assert paged.data(index, paged.TitleRole) == "PagedA"
        finally:
            view.deleteLater()
