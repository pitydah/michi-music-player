...
class TestHomePagePlayback:
    def test_shows_current_track(self):
        bridge = MagicMock()
        bridge.currentTrackTitle = "Canción de prueba"
        assert bridge.currentTrackTitle == "Canción de prueba"

    def test_shows_current_artist(self):
        bridge = MagicMock()
        bridge.currentArtist = "Artista de prueba"
        assert bridge.currentArtist == "Artista de prueba"

    def test_has_playback_flag(self):
        bridge = MagicMock()
        bridge.hasPlayback = True
        assert bridge.hasPlayback


class TestHomePageSignals:
    class _HomePageSignals(QObject):
        continuePlayback = Signal()
        resumeQueue = Signal()
        reconnectServer = Signal()
        openJobs = Signal()
        openAssistant = Signal()

    def test_continue_playback_signal(self):
        obj = self._HomePageSignals()
        fired = []
        obj.continuePlayback.connect(lambda: fired.append(True))
        obj.continuePlayback.emit()
        assert len(fired) == 1

    def test_resume_queue_signal(self):
        obj = self._HomePageSignals()
        fired = []
    ...
    def test_reconnect_server_signal(self):
        obj = self._HomePageSignals()
        fired = []
    ...
    def test_open_jobs_signal(self):
        obj = self._HomePageSignals()
        fired = []
    ...
    def test_open_assistant_signal(self):
        obj = self._HomePageSignals()
        fired = []
...
class TestHomePageAccessible:
    def test_accessible_role(self, engine):
        ...
        obj, _ = _create_context(engine, comp)
        try:
            assert obj.property("objectName") == "homePage"
        finally:
            obj.deleteLater()
