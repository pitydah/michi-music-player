...
def build_manifest(store, db_conn, device_id: str = "desktop") -> PlaylistSyncManifest:
    ...
    for s in summaries:
        ...
        entries.append(PlaylistSyncManifestEntry(
            id=str(s.id), name=s.name, description=s.description, updated_at=s.updated_at,
            sync_version=s.sync_version, is_smart=s.is_smart, rules_hash=rules_hash,
            cover_id=f"pl_{s.id}", track_count=s.track_count, duration=s.total_duration,
            tracks_hash=tracks_hash))
    return PlaylistSyncManifest(generated_at=time.time(), device_id=device_id, playlists=entries)
...
def build_delta(store, db_conn, since: float, device_id: str = "desktop") -> dict:
    ...
    return {"format": "michi.playlists.delta.v1", "since": since, "generated_at": time.time(),
            "changed": [p.__dict__ for p in changed], "deleted_ids": deleted}


def serialize_manifest_safe(manifest: PlaylistSyncManifest) -> dict:
    return {"format": manifest.format, "generated_at": manifest.generated_at, "device_id": manifest.device_id,
            "playlists": [{"id": p.id, "name": p.name, "description": p.description, "updated_at": p.updated_at,
                           "sync_version": p.sync_version, "is_smart": p.is_smart, "rules_hash": p.rules_hash,
                           "cover_id": p.cover_id, "track_count": p.track_count, "duration": p.duration,
                           "tracks_hash": p.tracks_hash} for p in manifest.playlists],
...
def _find_deleted_since(store, db_conn, since: float) -> list[int]:
    ...
    all_possible = db_conn.execute("SELECT id FROM playlists WHERE created_at < ?", (since,)).fetchall()
    return [r["id"] for r in all_possible if r["id"] not in current]
