"""Playlist export service — M3U, M3U8, TXT, CSV, JSON, portable pack."""
...
def export_txt(store, pid: int, path: str) -> dict:
    ...
    for i, t in enumerate(tracks, 1):
        ...
        lines.append(f"{i:4d}. {t.artist} — {t.title}  [{dur}]" + (f"  ({t.album})" if t.album else ""))
    _atomic_write(path, "\n".join(lines), encoding="utf-8")
    return {"ok": True, "path": path, "count": len(tracks)}
...
def export_csv(store, pid: int, path: str) -> dict:
    ...
    with open(path, "w", newline="", encoding="utf-8") as f:
        w = csv.writer(f)
        w.writerow(["position", "title", "artist", "album", "year", "genre", "duration", "filepath", "format", "quality"])
        for t in tracks:
            w.writerow([t.position, t.title, t.artist, t.album, t.year, t.genre, t.duration, t.filepath, t.ext.upper(), t.quality_kind])
    return {"ok": True, "path": path, "count": len(tracks)}


def export_michi_json(store, pid: int, path: str, include_filepaths: bool = True, safe_mobile: bool = False) -> dict:
    tracks = store.get_playlist_items(pid)
    pl = store.get_playlist(pid)
    track_list = []
    for t in tracks:
        entry = {"position": t.position, "track_id": t.track_id, "track_uid": t.track_uid,
                 "title": t.title, "artist": t.artist, "album": t.album, "year": t.year,
                 "genre": t.genre, "duration": t.duration, "ext": t.ext, "bitrate": t.bitrate,
                 "sample_rate": t.sample_rate, "bit_depth": t.bit_depth, "quality": t.quality_kind,
                 "content_hash": t.content_hash}
        if include_filepaths and not safe_mobile:
            entry["filepath"] = t.filepath
        track_list.append(entry)
    data = {"format": "michi.playlist.v1", "exported_at": time.time(),
            "playlist": {"id": pid, "name": pl["name"] if pl else "",
...
def export_portable_pack(store, pid: int, target_dir: str, options: PlaylistExportOptions | None = None) -> dict:
    tracks = store.get_playlist_items(pid)
    pl = store.get_playlist(pid)
    ...
    for i, t in enumerate(tracks, 1):
        ...
        try:
            ...
            errors.append(f"Copy: {t.filepath} → {e}")
    ...
    _write_portable_report(os.path.join(target_dir, "report.html"), pl, copied, len(tracks), errors)
    return {"ok": True, "path": target_dir, "copied": copied, "total": len(tracks), "errors": errors}
...
def _write_portable_report(path: str, pl: dict | None, copied: int, total: int, errors: list[str]):
    name = pl["name"] if pl else "Playlist"
    html = f"""<!DOCTYPE html><html><head><meta charset="utf-8"><title>Portable Pack: {name}</title>
...
<body><h1>Portable Pack: {name}</h1><p>Copied: <span class="ok">{copied}</span> / {total}</p>"""
    if errors:
        html += "<h3>Errors:</h3><ul>" + "".join(f"<li class='err'>{e}</li>" for e in errors) + "</ul>"
    html += "</body></html>"
