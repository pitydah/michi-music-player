...
def _days_ago(value) -> int:
    ...
    try: return int((time.time() - float(value)) / 86400)
    except (TypeError, ValueError): return 99999
...
def evaluate_rules(rules_json: str, db_conn) -> list[int]:
    ...
    if isinstance(rules_data, str):
        try: rules_data = json.loads(rules_data)
        except json.JSONDecodeError: return []
    if not isinstance(rules_data, list): return []
    definitions = []
    for r in rules_data:
        if isinstance(r, dict):
            definitions.append(PlaylistSmartRule(field=r.get("field", ""), operator=r.get("op", r.get("operator", "equals")), value=r.get("value", "")))
    return _apply_rules(definitions, db_conn)


def _apply_rules(rules: list[PlaylistSmartRule], db_conn) -> list[int]:
    if not rules: return []
    field_map = {"title": "m.title", "artist": "m.artist", "album": "m.album", "albumartist": "m.albumartist",
                 "genre": "m.genre", "year": "m.year", "ext": "m.ext", "bit_depth": "m.bit_depth",
                 "sample_rate": "m.sample_rate", "bitrate": "m.bitrate", "duration": "m.duration",
                 "bpm": "m.bpm", "key": "m.key", "replaygain": "m.replaygain_track",
                 "favorite": "f.track_id", "play_count": "COALESCE(h.cnt, 0)", "last_played": "MAX(h.played_at)",
                 "track_uid": "m.track_uid", "content_hash": "m.content_hash"}
    joins, wheres, params = [], [], []
    for rule in rules:
        ...
        if rule.field in ("play_count", "last_played"):
            joins.append("LEFT JOIN (SELECT track_id, COUNT(*) as cnt, MAX(played_at) as played_at FROM play_history GROUP BY track_id) h ON h.track_id = CAST(m.id AS TEXT)")
        if rule.operator in ("equals",):
            wheres.append(f"{col} = ?"); params.append(rule.value)
        elif rule.operator == "contains":
            wheres.append(f"{col} LIKE ?"); params.append(f"%{rule.value}%")
        elif rule.operator == "not_contains":
            wheres.append(f"{col} NOT LIKE ?"); params.append(f"%{rule.value}%")
        elif rule.operator == "greater_than":
            wheres.append(f"{col} > ?"); params.append(float(rule.value))
    ...
    if not wheres:
        sql = "SELECT DISTINCT m.id FROM media_items m" + "".join(f" {j}" for j in joins)
        return [r["id"] for r in db_conn.execute(sql).fetchall()]
    sql = "SELECT DISTINCT m.id FROM media_items m" + "".join(f" {j}" for j in joins) + " WHERE " + " AND ".join(wheres) + " ORDER BY m.title COLLATE NOCASE"
    try:
        return [r["id"] for r in db_conn.execute(sql, params).fetchall()]
...
def create_smart_playlist(store, name: str, rules_json: str, description: str = ""):
    return store.create_playlist(name, description=description, is_smart=True, rules_json=rules_json)


def refresh_smart_playlist(store, pid: int, db_conn) -> int:
    pl = store.get_playlist(pid)
    if not pl or not pl.get("is_smart"): return 0
    track_ids = evaluate_rules(pl.get("rules_json", ""), db_conn)
    store.clear_playlist(pid)
    for tid in track_ids:
        row = db_conn.execute("SELECT filepath FROM media_items WHERE id=?", (tid,)).fetchone()
        if row:
...
def convert_mix_to_playlist(store, db_conn, mix_key: str, name: str | None = None) -> dict | None:
